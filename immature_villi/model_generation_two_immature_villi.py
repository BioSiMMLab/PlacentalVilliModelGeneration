...
b = 0.144 #ellipse semi-minor axis (mm) 
...
height = 1.13   #height of the elliptical cylinder (mm)
z_slice = 0.6     # in mm
...
z_branch = np.array([0.305, 0.38, 0.46, 0.50, 0.60, 0.65, 0.70, 0.80, 0.87, 0.96, 1.02, 1.08])
r_branch = np.array([0.035, 0.045, 0.05, 0.06, 0.07, 0.07, 0.07, 0.07, 0.07, 0.06, 0.05, 0.045])
...
x_ms = x_main[3]
x_branch = np.array([ x_ms, 0.75 * x_ms + 0.25 * x_rand_b, 0.5 * x_ms + 0.5 * x_rand_b, 0.25 * x_ms + 0.75 * x_rand_b, x_rand_b, x_rand_b + 0.01, x_rand_b+ 0.01, x_rand_b + 0.01,
    x_rand_b+ 0.01, x_rand_b + 0.015, x_rand_b + 0.015, x_rand_b + 0.01])
...
branch_path_smooth = smooth_path(branch_path, s=0.001 , num=60)
...
smoothed = smooth_junction(main_fillet , branch_fillet, fused, z_min=0.25, z_max=0.55, radius=0.015)

...
b = 0.1225 #ellipse semi-minor axis (mm) 
i = 1   # The ID of the sample
output_dir = f"geometries_b{b:.4f}mm/{i}/Geometry"
...
z_main = np.array([-0.01, 0.11, 0.19, 0.29, 0.4, 0.45, 0.5, 0.55, 0.6, 0.66, 0.72, 0.77, 0.85, 0.91, 0.96, 1.01, 1.11])
r_main = np.array([0.06, 0.06, 0.06, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.04])

z_branch = np.array([0.305, 0.38, 0.46, 0.50, 0.60, 0.65, 0.70, 0.80, 0.87, 0.96, 1.02, 1.11])
...
x_main = np.array([ 0.6 * x_rand_m + 0.4 * x_rand_b , 0.6 * x_rand_m + 0.4 * x_rand_b, 0.6 * x_rand_m + 0.4 * x_rand_b, 0.75 * x_rand_m + 0.25 * x_rand_b , 0.9*x_rand_m+0.1*x_rand_b , x_rand_m, x_rand_m,
    x_rand_m, x_rand_m+0.01, x_rand_m, x_rand_m +0.015, x_rand_m + 0.015, x_rand_m, x_rand_m + 0.015, x_rand_m, x_rand_m + 0.01, x_rand_m])
...
main_path_smooth = smooth_path(main_path, s=0.0 , num=60)
...
smoothed = smooth_junction(main_fillet, branch_fillet , fused, z_min=0.25, z_max=0.55, radius=0.01)

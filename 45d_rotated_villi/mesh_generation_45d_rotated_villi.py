...
for i in range(1, 26):
    ...
    for tag in candidates:
        ...
        for u, v in [(0.25,0.25), (0.5,0.25), (0.25,0.5)]:
            try:
                x, y, z = gmsh.model.getValue(2, tag, [u,v])
                val = (x**2)/(a**2) + (y**2)/(b**2)
                if z < 0.05 and val > (1-epsilon): 
                    is_periphery = True
                    break
            except:
                continue
    ...
    #applying distance-based mesh refinement around the central villi
    villi_field = gmsh.model.mesh.field.add("Distance")
    gmsh.model.mesh.field.setNumbers(villi_field, "FacesList", refined_villi_tags)   
    ...
    gmsh.model.mesh.field.setNumber(thresh_field,"DistMax", 0.03)
